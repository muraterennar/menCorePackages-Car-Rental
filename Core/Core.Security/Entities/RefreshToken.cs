...
namespace MenCore.Security.Entities
{
    ...
    public class RefreshToken : Entity<int>
    {
        ...
        public RefreshToken(int id, DateTime createdDate, DateTime? updatedDate, DateTime? deletedDate) : base(id, createdDate, updatedDate, deletedDate)
        {
        }

        // Parametreleri kullanarak bir yenileme token'i oluşturan yapılandırıcı
        public RefreshToken(int userId, string token, DateTime expires, string createdByIp, DateTime? revoked, string? revokedByIp, string? replacedByToken, string? reasonRevoked, User user) : this(userId)
        {
            Token = token;
            Expires = expires;
